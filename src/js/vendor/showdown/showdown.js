...
// exposed is makeHtml().
//
// accepts an array of config options that will merge with the config array below
...
// (see _ProcessListItems() for details):
    ...
    var config = this.config = {
        ...
        additional_parser: false
    }

// loop through each argument passed and set it as a config option
    if (arguments[0]) {
        for (x in arguments[0]) {
            this.config[x] = arguments[0][x];
        }
    }
...
//
        ...
        if (typeof config.additional_parser == 'function') {
            text = config.additional_parser(text);
        }
